...
1. Configure Snort to log alerts in alert_csv format (alert_fast style) to a process that posts to the backend. Example using `tail -F` and `curl`:
...
```sh
#!/bin/sh
BACKEND=http://<backend-host>:4000/api/logs
...
4. Production forwarder (recommended): use the Node forwarder included in `backend-api/tools/forwarder.js` which supports batching, retry with exponential backoff, and graceful shutdown. Example:

```
node backend-api/tools/forwarder.js /var/log/snort/alert_csv.log http://backend-host:4000/api/logs YOUR_API_KEY
```
